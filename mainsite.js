/* ==========================================================================
   ...
   31. Live market widgets (BTC & ETH candlestick charts)
   ========================================================================== */
...
function skeletonRowsHtml(count) {
  ...
  for (let i = 0; i < count; i++) {
    html +=
      '<div class="skeleton-row"><div class="skeleton-circle"></div><div class="skeleton-lines"><div class="skeleton-line short"></div><div class="skeleton-line long"></div></div></div>';
  }
  return html;
}
...
function selectWithdrawalNetwork(net) {
  ...
  if (savedWallet) {
    input.readOnly = true;
    note.textContent =
      'This wallet is locked to your account. To change it, go to Settings from your account menu.';
  } else {
    input.readOnly = false;
    note.textContent =
      'No saved ' +
      net +
      ' wallet found on your account. Enter one below — it will be locked to your account once saved.';
  }
}
/* -------------------------- 8. Auth modal open/close/switch helpers -------------------------- */
...
async function updatePassword() {
  ...
  try {
    ...
    console.error(err);
    showMsg('newPasswordMsg', 'Unable to update password right now. Please try again.');
  } finally {
    if (button) {
      button.disabled = false;
...
function friendlySignupError(msg) {
  ...
  if (
    m.includes('profiles_wallet_address_unique') ||
    (m.includes('wallet') && m.includes('duplicate'))
  ) {
...
async function signup() {
  ...
  if (!supabaseReady) {
    showMsg('signupMsg', 'Connection is not ready. Please refresh the page and try again.');
    return;
  }
  ...
  if (!email || !pass) {
    showMsg('signupMsg', 'Please fill Email and Password.');
    return;
  }
  ...
  try {
    ...
    if (error) {
      showMsg('signupMsg', friendlySignupError(error.message));
      return;
    }
    ...
    showMsg('signupMsg', err?.message || 'Unable to create your account. Please try again.');
  } finally {
    if (button) {
      button.disabled = false;
...
async function login() {
  clearMessages();
  if (!supabaseReady) {
    showMsg('loginMsg', 'Connection is not ready. Please refresh the page and try again.');
    return;
  }
  const email = $('loginEmail').value.trim();
  const password = $('loginPassword').value;
  if (!email || !password) {
    showMsg('loginMsg', 'Enter email and password.');
    return;
  }
  ...
  try {
    ...
    console.error(err);
    showMsg('loginMsg', 'Unable to login right now. Please try again.');
  } finally {
    if (button) {
      button.disabled = false;
...
async function loadDashboard() {
  ...
  try {
    ...
    /* Shimmer placeholders while profile/account load; cleared by
       renderAccountSummary() and loadReferralInfo() once real data is in. */
    showDashboardSkeleton();

    /* ================= PROFILE ================= */

    /* notifications_seen_at = last time the client bulk-cleared notifications.
       read_notification_keys = JSONB array of individually-opened notification
       keys (lets a single click mark just that one as read — see section 21).
       Both live on the profile row so read state follows the client across
       devices. first_name/last_name/address/city/postal_code/state are the
       profile-completion fields (section 13.5) required before a deposit. */
    const { data: profile, error: profileError } = await supabaseClient
      .from('profiles')
      .select(
        'full_name,first_name,last_name,phone,address,city,postal_code,state,wallet_address,wallet_address_bep20,notifications_seen_at,read_notification_keys',
      )
      .eq('id', user.id)
      .maybeSingle();
    ...
    /* Prefer the profile name; fall back to Auth user metadata. */
    ...
    /* If Auth already has the client's info, make sure the profile row does too. */
            ...
            if (updatedProfile) {
              ...
              const updatedName =
                (updatedProfile.first_name
                  ...
                  : updatedProfile.full_name?.trim()) || displayName;
...
/* Signup only asks for email + password (section 11) — every other client
   detail (name, phone, address, wallets) is collected here instead, the
   first time the client tries to make a deposit.
   
   isProfileComplete() is the single gate: openRequest('deposit') calls it
   before showing the deposit modal, and shows this profile-completion modal
   instead if anything required is missing. Withdrawals aren't gated here
   since a client can't have profit before their first deposit anyway.
   
   Required: first name, last name, phone, address, city, postal code,
   state, and at least one of the two wallets. */
...
/* Locks a wallet input in the Complete Profile / Settings form once that
   network's wallet already has a saved value. Called from openCompleteProfile(). */
...
/* Opens the profile-completion modal, pre-filled with whatever is already
   on file. */
...
/* Validates every field, saves the profile in one update, keeps full_name
   in sync, then reopens the deposit modal if this was triggered by "Deposit". */
async function submitProfile() {
  clearMessages();
  if (!supabaseReady) {
    showMsg('profileMsg', 'Connection is not ready. Please refresh the page and try again.');
    return;
  }
  if (!currentUser) {
    showMsg('profileMsg', 'Please login again.');
    return;
  }
  ...
  if (!firstName || !lastName || !phone || !address || !city || !postalCode || !state) {
    showMsg('profileMsg', 'Please fill all fields.');
    return;
  }
  if (!walletTrc20 && !walletBep20) {
    showMsg(
      'profileMsg',
      'Please provide at least one withdrawal wallet address (TRC20 or BEP20).',
    );
    return;
  }
  if (walletTrc20 && !validWallet(walletTrc20)) {
    showMsg(
      'profileMsg',
      'Please enter a valid TRC20 wallet address starting with T, or leave it blank.',
    );
    return;
  }
  if (walletBep20 && !validBep20Wallet(walletBep20)) {
    showMsg(
      'profileMsg',
      'Please enter a valid BEP20 wallet address starting with 0x, or leave it blank.',
    );
    return;
  }
  ...
  try {
    ...
    if (error) {
      console.error(error);
      showMsg('profileMsg', friendlySignupError(error.message));
      return;
    }
    ...
    console.error(err);
    showMsg('profileMsg', 'Unable to save profile right now.');
  } finally {
    if (button) {
      button.disabled = false;
...
/* Sums this user's currently pending withdrawal requests. Without
   subtracting these, a client could submit several withdrawal requests
   back-to-back that each look valid against the raw balance/profit but
   together exceed what's actually available. */
...
/* Available amount = (profit share, or full balance once principal is
   ...
   requests. Async since it queries Supabase. */
...
async function renderAccountSummary(a) {
  ...
  if (days === null) {
    ...
    noteEl.textContent =
      'Once you make your first deposit, it unlocks for withdrawal after 40 days. Your profit can be withdrawn any time before that.';
  } else if (days >= PRINCIPAL_LOCK_DAYS) {
    fillEl.style.width = '100%';
    fillEl.classList.add('done');
    daysTextEl.textContent = 'Unlocked ✅';
    noteEl.textContent =
      'Your deposited amount is fully unlocked — your whole balance is available to withdraw.';
  } else {
    ...
    noteEl.textContent =
      'Your deposited amount unlocks in ' +
      daysLeft +
      ' day' +
      (daysLeft === 1 ? '' : 's') +
      '. Your profit share is available to withdraw right now.';
  }
  ...
  if (type === 'withdrawal') {
    ...
    if (noteEl) {
      if (principalUnlocked) {
        noteEl.textContent =
          'Your principal is unlocked. You can withdraw up to $' + avail.toFixed(2) + ' now.';
      } else {
        const daysLeft = Math.max(0, Math.ceil(PRINCIPAL_LOCK_DAYS - (days || 0)));
        noteEl.textContent =
          'You can withdraw up to $' +
          avail.toFixed(2) +
          ' now (your profit share, after accounting for any pending requests). Your deposited capital unlocks in ' +
          daysLeft +
          ' day' +
          (daysLeft === 1 ? '' : 's') +
          '.';
...
async function submitDeposit() {
  clearRequestMessages();
  if (!supabaseReady) {
    showMsg('depositMsg', 'Connection is not ready. Please refresh the page and try again.');
    return;
  }
  if (!currentUser) {
    showMsg('depositMsg', 'Please login again.');
    return;
  }
  ...
  if (!Number.isFinite(amount) || amount < 100) {
    showMsg('depositMsg', 'Minimum deposit is $100.');
    return;
  }
  if (!tx) {
    showMsg('depositMsg', 'Please enter the transaction hash.');
    return;
  }
  if (!proofFile) {
    showMsg('depositMsg', 'Please upload a screenshot of your payment.');
    return;
  }
  if (proofFile.size > 5 * 1024 * 1024) {
    showMsg('depositMsg', 'Screenshot must be under 5MB.');
    return;
  }
  ...
  try {
    ...
    if (uploadError) {
      ...
      showMsg('depositMsg', 'Screenshot upload failed: ' + uploadError.message);
      return;
    }
    ...
    if (error) {
      console.error(error);
      showMsg('depositMsg', error.message);
      return;
    }
    showMsg(
      'depositMsg',
    ...
    console.error(err);
    showMsg('depositMsg', 'Unable to submit deposit request right now.');
  } finally {
    if (button) {
      button.disabled = false;
...
async function submitWithdrawal() {
  clearRequestMessages();
  if (!supabaseReady) {
    showMsg('withdrawalMsg', 'Connection is not ready. Please refresh the page and try again.');
    return;
  }
  if (!currentUser) {
    showMsg('withdrawalMsg', 'Please login again.');
    return;
  }
  ...
  if (!Number.isFinite(amount) || amount <= WITHDRAWAL_FEE) {
    showMsg(
      'withdrawalMsg',
      'Withdrawal amount must be greater than the $' + WITHDRAWAL_FEE.toFixed(2) + ' fee.',
    );
    return;
  }
  ...
  if (network === 'TRC20') {
    if (!validWallet(wallet)) {
      showMsg('withdrawalMsg', 'Please enter a valid TRC20 wallet address starting with T.');
      return;
    }
  } else {
    if (!validBep20Wallet(wallet)) {
      showMsg('withdrawalMsg', 'Please enter a valid BEP20 wallet address starting with 0x.');
      return;
    }
  }
  ...
  const { data: accounts, error: accountError } = await supabaseClient
    .from('accounts')
    .select('balance,profit,first_deposit_at')
    .eq('user_id', currentUser.id)
    .limit(1);
  if (accountError) {
    showMsg('withdrawalMsg', accountError.message);
    return;
  }
  ...
  if (amount > balance) {
    showMsg(
      'withdrawalMsg',
      'Withdrawal amount is higher than your available balance of $' + balance.toFixed(2) + '.',
    );
    return;
  }

  /* Re-checks pending withdrawals fresh, right before insert. Paired with a
     DB-level trigger (check_withdrawal_amount) so this can't be bypassed by
  ...
  if (amount > maxWithdrawable) {
    ...
    if (maxWithdrawable <= 0) {
      showMsg(
        'withdrawalMsg',
        'You have no available balance to withdraw right now — you may already have a pending withdrawal request awaiting review.',
      );
    } else {
      showMsg(
        'withdrawalMsg',
        'You can currently withdraw up to $' +
          maxWithdrawable.toFixed(2) +
          ' (after accounting for any pending requests). Your deposited capital unlocks in ' +
          daysLeft +
          ' day' +
          (daysLeft === 1 ? '' : 's') +
          '.',
  ...
  try {
    ...
    if (error) {
      console.error(error);
      showMsg('withdrawalMsg', error.message);
      return;
    }
    showMsg(
      'withdrawalMsg',
      'Withdrawal request submitted successfully. It is pending review. You will receive $' +
        netAmount.toFixed(2) +
        ' after the $' +
        fee.toFixed(2) +
        ' fee.',
    ...
    console.error(err);
    showMsg('withdrawalMsg', 'Unable to submit withdrawal request right now.');
  } finally {
    if (button) {
      button.disabled = false;
...
async function loadNotifications() {
  ...
  try {
    const [d, w, p, tc] = await Promise.all([
      ...
      supabaseClient
        .from('profit_entries')
        .select('client_share,entry_date,created_at')
        .eq('user_id', currentUser.id)
        .order('created_at', { ascending: false })
        .limit(10),
      supabaseClient
        .from('trade_code_applications')
        .select('adjustment_amount,adjustment_type,percentage,code_snapshot,applied_at')
        .eq('user_id', currentUser.id)
    ...
    (d.data || []).forEach((x) => {
      ...
      items.push({
        icon,
        text: statusText + ' — $' + Number(x.amount).toFixed(2),
        date: x.created_at,
        key: 'deposit-' + x.status + '-' + x.created_at,
      });
    });
    ...
    (w.data || []).forEach((x) => {
      ...
      items.push({
        ...
        text: statusText + ' — $' + Number(x.amount).toFixed(2),
        date: x.created_at,
        key: 'withdrawal-' + x.status + '-' + x.created_at,
      });
    });
    ...
    (p.data || []).forEach((x) => {
      items.push({
        icon: '📈',
        text: 'Daily profit updated — +$' + Number(x.client_share || 0).toFixed(2),
        date: x.created_at || x.entry_date,
        key: 'profit-' + (x.created_at || x.entry_date),
      });
    });
    ...
    (tc.data || []).forEach((x) => {
      ...
      const label = isIncrease
        ? 'Trade code applied — profit added'
        : 'Trade code applied — loss applied';
      items.push({
        icon,
...
/* Prints a percentage cleanly (e.g. "1" instead of "1.0000"). */
function esc_pct(n) {
...
/* Opens/closes the small centered popup (+ its dim backdrop). Reloads the
   feed each time it opens so a freshly-arrived notification is reflected
   immediately. */
...
/* Opens one notification's own popup on top of the list, and marks this
   specific notification as individually read (see markNotificationRead). */
...
/* Marks one notification (by its unique key) as individually read.
   Persists into profiles.read_notification_keys, updates the local
   currentProfile copy, and re-renders the list immediately. */
async function markNotificationRead(key) {
  if (!currentUser || !supabaseReady || !key) return;

  const existing = Array.isArray(currentProfile?.read_notification_keys)
...
/* The dim backdrop is shared by the list and the detail popup — a click on
   it should only close whichever layer is on top. */
...
/* Persists the bulk "seen" timestamp to the profile row (so it's shared
   across devices), resets read_notification_keys to [], and updates the
   UI immediately. */
...
/* Opened by clicking the avatar/name block in the dashboard topbar.
   - View Profile   → read-only summary from currentProfile.
   - Settings       → reopens the profile-completion form (openCompleteProfile()).
   - Team           → clients this client referred, via get_referred_clients() RPC.
   - Invite Members → opens #inviteModal (referral link, Copy/Share, stats, terms).
   - Support        → message form inserting into contact_messages.
   - Sign Out       → reuses logout(). */
...
function toggleAccountMenu() {
  ...
  if (willShow) {
    ...
    const headName = $('menuHeadName');
    if (headName) headName.textContent = name;
    const headEmail = $('menuHeadEmail');
    if (headEmail) headEmail.textContent = currentUser?.email || '—';
    const headAvatar = $('menuHeadAvatar');
    if (headAvatar) headAvatar.textContent = (name.trim().charAt(0) || 'C').toUpperCase();
  }
...
/* Every value rendered into these popups comes from client-typed data, so
...
function openViewProfile() {
  ...
  if (list) {
    list.innerHTML = rows
      .map(
        ([label, value]) =>
          '<div class="profile-view-row"><span class="profile-view-label">' +
          label +
          '</span><span class="profile-view-value">' +
...
async function openTeam() {
  ...
  try {
    ...
    if (!rows.length) {
      if (list)
        list.innerHTML =
          '<div class="tx-empty">No referrals yet — share your link from Invite Members.</div>';
      return;
    }
    if (list)
      list.innerHTML = rows
        .map(
          (r) =>
            '<div class="team-item">' +
            '<div><div class="team-item-name">' +
            escapeHtml(r.name || 'Client') +
            '</div>' +
            '<div class="team-item-date">Joined ' +
            new Date(r.joined_at).toLocaleDateString() +
            '</div>' +
            '<div class="team-item-earn">Deposited $' +
            Number(r.total_deposited || 0).toFixed(2) +
            ' · Commission $' +
            Number(r.commission_earned || 0).toFixed(2) +
            '</div>' +
            '</div>' +
            '<span class="tx-status ' +
            (r.deposit_approved ? 'status-approved' : 'status-pending') +
            '">' +
            (r.deposit_approved ? 'Deposited' : 'No deposit yet') +
            '</span>' +
            '</div>',
        )
        .join('');
...
async function submitSupportMessage() {
  if (!supabaseReady) {
    showMsg('supportMsg', 'Connection is not ready. Please refresh the page and try again.');
    return;
  }
  if (!currentUser) {
    showMsg('supportMsg', 'Please login again.');
    return;
  }
  ...
  if (!subject || !message) {
    showMsg('supportMsg', 'Please fill both subject and message.');
    return;
  }
  if (attachmentFile && attachmentFile.size > 5 * 1024 * 1024) {
    showMsg('supportMsg', 'Attachment must be under 5MB.');
    return;
  }
  ...
  try {
    ...
    if (attachmentFile) {
      ...
      if (uploadError) {
        console.error(uploadError);
        showMsg('supportMsg', 'Attachment upload failed: ' + uploadError.message);
        return;
      }
    ...
    if (error) {
      console.error(error);
      showMsg('supportMsg', error.message);
      return;
    }
    ...
    console.error(err);
    showMsg('supportMsg', 'Unable to send your message right now.');
  } finally {
    if (btn) {
      btn.disabled = false;
...
function updateCalculator() {
  ...
  $('outClient').textContent = (v < 0 ? '-' : '') + '$' + Math.abs(v * 0.6).toFixed(2);
  $('outMgr').textContent = (v < 0 ? '-' : '') + '$' + Math.abs(v * 0.4).toFixed(2);
}
...
/* Referral flow:
   ...
   per deposit, ever (unique constraint on the SQL side).
   ...
   6. Commission rate is a single admin-editable setting — DEFAULT_REFERRAL_RATE
   above is only a display fallback until the real rate loads from
   ...
   with the client's own link, stats, and copy/share actions. */

/* Reads ?ref=CODE from the URL and remembers it in sessionStorage so it
   survives from landing page to signup submission. Called once on page load. */
...
/* Loads this client's referral dashboard data via get_referral_dashboard()
   and renders the locked/unlocked card. Expected shape:
   { referral_code, referral_unlocked, referrals_count,
   referred_deposits_total, earnings_total, commission_rate } */
async function loadReferralInfo() {
  ...
  try {
    ...
    if (unlocked) {
      ...
      if (depositsEl)
        depositsEl.textContent = '$' + Number(info.referred_deposits_total || 0).toFixed(2);
...
function shareReferralLink() {
  ...
  if (navigator.share) {
    navigator
      .share({ title: 'PipZoNe', text: 'Join PipZoNe using my referral link:', url: field.value })
      .catch(() => {});
  } else {
    copyReferralLink();
  }
...
document.addEventListener('DOMContentLoaded', async function () {
  ...
  if (supportAttachmentInput)
    supportAttachmentInput.addEventListener('change', () => {
  ...
  document
    ...
    .forEach((q) => q.addEventListener('click', () => q.parentElement.classList.toggle('open')));

  document
    .querySelectorAll(
      '#loginForm input,#signupForm input,#resetPasswordForm input,#newPasswordForm input',
    )
    .forEach((input) =>
      input.addEventListener('keydown', (e) => {
        if (e.key === 'Enter') {
          ...
          if ($('loginForm').style.display !== 'none') {
            ...
            sendResetEmail();
          } else if ($('newPasswordForm').style.display !== 'none') {
            updatePassword();
          }
        }
      }),
  ...
  document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') {
      ...
      if ($('notifDetailModal')?.classList.contains('show')) {
        closeNotifDetail();
      } else {
        closeNotifications();
      }
  ...
  document.querySelectorAll('.modal,.request-modal').forEach((m) =>
